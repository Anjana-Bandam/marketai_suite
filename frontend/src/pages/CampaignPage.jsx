...
// import { Megaphone, Copy, Check, Sparkles, ChevronDown, Download } from 'lucide-react'
...
// // Poster color themes
// const posterThemes = [
//   { bg: 'linear-gradient(135deg, #1a1714 0%, #3d2b1f 100%)', accent: '#c94f1e', text: '#f5f0e8', sub: 'rgba(245,240,232,0.6)' },
//   { bg: 'linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%)', accent: '#00d4ff', text: '#ffffff', sub: 'rgba(255,255,255,0.6)' },
//   { bg: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', accent: '#e94560', text: '#ffffff', sub: 'rgba(255,255,255,0.6)' },
//   { bg: 'linear-gradient(135deg, #0d0d0d 0%, #1a1a1a 100%)', accent: '#39ff14', text: '#ffffff', sub: 'rgba(255,255,255,0.55)' },
//   { bg: 'linear-gradient(135deg, #2d1b69 0%, #11998e 100%)', accent: '#f7971e', text: '#ffffff', sub: 'rgba(255,255,255,0.6)' },
// ]

// function extractPosterData(campaignText, product, audience) {
//   const lines = campaignText.split('\n').filter(l => l.trim())
//   // Extract headline ideas
//   const headlineIdx = lines.findIndex(l => l.toLowerCase().includes('headline'))
//   let headlines = []
//   if (headlineIdx !== -1) {
//     for (let i = headlineIdx + 1; i < Math.min(headlineIdx + 5, lines.length); i++) {
...
// function CampaignPoster({ campaignText, product, audience, themeIdx = 0 }) {
//   const theme = posterThemes[themeIdx % posterThemes.length]
//   const data = extractPosterData(campaignText, product, audience)
...
//       <div style={{ position:'absolute', top:-60, right:-60, width:240, height:240, borderRadius:'50%', background:`${theme.accent}15`, pointerEvents:'none' }} />
//       <div style={{ position:'absolute', bottom:-80, left:-40, width:300, height:300, borderRadius:'50%', background:`${theme.accent}08`, pointerEvents:'none' }} />
//       <div style={{ position:'absolute', top:'40%', left:'60%', width:120, height:120, borderRadius:'50%', background:`${theme.accent}10`, pointerEvents:'none' }} />
...
//           <div style={{ width:32, height:32, borderRadius:8, background:theme.accent, display:'flex', alignItems:'center', justifyContent:'center' }}>
//             <span style={{ fontSize:16 }}>⚡</span>
//           </div>
//           <span style={{ color:theme.text, fontWeight:700, fontSize:13, letterSpacing:'0.5px' }}>MarketAI Suite</span>
//         </div>
//         <div style={{ background:`${theme.accent}20`, border:`1px solid ${theme.accent}40`, borderRadius:100, padding:'4px 14px', fontSize:11, color:theme.accent, fontWeight:600, letterSpacing:'1px', textTransform:'uppercase' }}>
//           Campaign
//         </div>
//       </div>
...
//         {/* Accent line */}
//         <div style={{ width:48, height:4, background:theme.accent, borderRadius:100, marginBottom:20 }} />
...
//         <p style={{ fontSize:14, color:theme.sub, lineHeight:1.65, maxWidth:'80%', marginBottom:24 }}>
//           {data.subheadline}
//         </p>
...
//                 <div style={{ width:6, height:6, borderRadius:'50%', background:theme.accent, marginTop:6, flexShrink:0 }} />
//                 <span style={{ fontSize:13, color:theme.sub, lineHeight:1.5 }}>{b}</span>
//               </div>
//             ))}
//           </div>
//         )}
...
//         <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:`${theme.accent}15`, border:`1px solid ${theme.accent}30`, borderRadius:100, padding:'6px 16px', marginBottom:28 }}>
//           <span style={{ fontSize:11, color:theme.accent, fontWeight:600 }}>🎯 {data.audience}</span>
//         </div>
//       </div>
...
//           <div style={{ background:theme.accent, borderRadius:100, padding:'12px 28px', fontSize:13, fontWeight:700, color:'#fff', letterSpacing:'0.3px', display:'inline-block' }}>
//             {data.cta.length > 40 ? data.cta.slice(0, 40) + '...' : data.cta}
//           </div>
//           <div style={{ textAlign:'right' }}>
//             <div style={{ fontSize:10, color:theme.sub, letterSpacing:'1px', textTransform:'uppercase' }}>Powered by</div>
//             <div style={{ fontSize:12, color:theme.text, fontWeight:600 }}>MarketAI Suite</div>
//           </div>
//         </div>
//       </div>
//     </div>
...
//   const [posterTheme, setPosterTheme] = useState(0)
...
//   const copy = () => {
//     if (result?.campaign_content) { navigator.clipboard.writeText(result.campaign_content); setCopied(true); setTimeout(() => setCopied(false), 2000) }
//   }
...
//         .theme-dot.active{border-color:#1a1714;transform:scale(1.25)}
...
//               </div>
//             </div>
//             {error && <div style={{ background:'rgba(201,79,30,0.06)', border:'1px solid rgba(201,79,30,0.2)', borderRadius:12, padding:'12px 16px', fontSize:13, color:'#c94f1e', marginBottom:20 }}>{error}</div>}
...
//                 </div>
//                 <button onClick={copy} style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(26,23,20,0.05)', border:'1px solid rgba(26,23,20,0.1)', borderRadius:100, padding:'7px 16px', fontSize:12, color:'#1a1714', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", transition:'all 0.15s' }}>
...
//                 <h3 style={{ fontFamily:"'Fraunces',serif", fontSize:22, fontWeight:900, letterSpacing:'-1px', color:'#1a1714' }}>Campaign Visual 🎨</h3>
//               </div>
//               <div style={{ display:'flex', alignItems:'center', gap:12 }}>
//                 {/* Theme picker */}
//                 <div style={{ display:'flex', alignItems:'center', gap:6 }}>
//                   {posterThemes.map((t, i) => (
//                     <div
//                       key={i}
//                       className={`theme-dot ${posterTheme === i ? 'active' : ''}`}
//                       style={{ background: t.bg.includes('135deg') ? t.accent : t.accent }}
//                       onClick={() => setPosterTheme(i)}
//                       title={`Theme ${i+1}`}
//                     />
//                   ))}
//                 </div>
...
//                   style={{ display:'flex', alignItems:'center', gap:6, background:'#1a1714', border:'none', borderRadius:100, padding:'10px 22px', fontSize:13, fontWeight:600, color:'#f5f0e8', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", transition:'all 0.2s', opacity: downloadingPoster ? 0.6 : 1 }}
//                 >
//                   <Download size={13} />
...
//             <CampaignPoster
//               campaignText={result.campaign_content}
//               product={form.product}
//               audience={form.target_audience}
//               themeIdx={posterTheme}
//             />
//           </div>
//         )}
...
// const posterLayouts = [
//   // Layout 0: Bold left-aligned, big headline
//   ({ data, image, accent, textColor, subColor }) => (
//     <div id="campaign-poster" style={{
...
//   // Layout 1: Split — dark left panel + image right
//   ({ data, image, accent, textColor, subColor }) => (
//     <div id="campaign-poster" style={{
...
//           ))}
//         </div>
//         <div style={{ background:accent, borderRadius:100, padding:'10px 22px', fontSize:11, fontWeight:700, color:'#fff', display:'inline-block', alignSelf:'flex-start' }}>{data.cta.slice(0,32)}</div>
//       </div>
//       <div style={{ position:'relative' }}>
//         {image && <img src={image} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />}
//         <div style={{ position:'absolute', inset:0, background:`linear-gradient(135deg, ${accent}30, transparent)` }} />
//         <div style={{ position:'absolute', bottom:20, right:20, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(8px)', borderRadius:100, padding:'6px 14px', fontSize:9, color:'rgba(255,255,255,0.7)', fontWeight:600, letterSpacing:'1px' }}>🎯 {data.audience.slice(0,30)}</div>
//       </div>
//     </div>
//   ),

//   // Layout 2: Centered magazine style
//   ({ data, image, accent, textColor, subColor }) => (
//     <div id="campaign-poster" style={{
//       position:'relative', borderRadius:20, overflow:'hidden',
...
//   // Layout 3: Top image, bottom content card
//   ({ data, image, accent, textColor, subColor }) => (
//     <div id="campaign-poster" style={{
...
//         {image && <img src={image} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />}
//         <div style={{ position:'absolute', inset:0, background:`linear-gradient(to bottom, transparent 40%, rgba(26,23,20,0.9) 100%)` }} />
...
//           <div style={{ background:'rgba(245,240,232,0.07)', border:`1px solid ${accent}40`, borderRadius:100, padding:'6px 14px', fontSize:10, color:`${accent}`, fontWeight:600 }}>🎯 {data.audience.slice(0,24)}</div>
//         </div>
//       </div>
//     </div>
//   ),

//   // Layout 4: Neon/vibrant full overlay
//   ({ data, image, accent, textColor, subColor }) => (
//     <div id="campaign-poster" style={{
//       position:'relative', borderRadius:20, overflow:'hidden',
//       minHeight:500, fontFamily:"'DM Sans',sans-serif", boxShadow:'0 20px 60px rgba(0,0,0,0.35)',
//     }}>
//       {image && <img src={image} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />}
//       <div style={{ position:'absolute', inset:0, background:`linear-gradient(135deg, ${accent}ee 0%, rgba(0,0,0,0.85) 100%)` }} />
...
//       <div style={{ position:'absolute', bottom:0, right:0, width:250, height:250, borderRadius:'50%', background:'rgba(255,255,255,0.04)', transform:'translate(60px,60px)' }} />
//       <div style={{ position:'relative', zIndex:1, padding:'44px 40px', height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
//         <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
//           <div style={{ display:'flex', alignItems:'center', gap:8 }}>
...
//       // Pick random layout and accent every generation
...
//       const imageQuery = `${form.product} ${form.target_audience} business`
...
//     setPosterLayout(Math.floor(Math.random() * posterLayouts.length))
//     setPosterAccent(accentColors[Math.floor(Math.random() * accentColors.length)])
//     const imageQuery = `${form.product} ${form.target_audience} professional`
//     const img = await fetchUnsplashImage(imageQuery)
//     setPosterImage(img)
//     setLoadingPoster(false)
//   }
...
//                   style={{ display:'flex', alignItems:'center', gap:6, background:'#1a1714', border:'none', borderRadius:100, padding:'10px 22px', fontSize:13, fontWeight:600, color:'#f5f0e8', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", opacity: downloadingPoster ? 0.6 : 1 }}
//                 >
//                   <Download size={13} />
//                   {downloadingPoster ? 'Saving...' : 'Download Poster'}
//                 </button>
...
//               </div>
//             ) : (
//               posterData && <PosterLayout data={posterData} image={posterImage} accent={posterAccent} />
//             )}

//             <div style={{ marginTop:12, fontSize:11, color:'#aaa09a', fontFamily:"'DM Sans',sans-serif", textAlign:'right' }}>
