...
//         persona: form.target_customer,      // frontend: target_customer → backend: persona
...
//     { label:'Elevator Pitch', content: result.elevator_pitch, color:'#c94f1e' },
//     { label:'Value Proposition', content: result.value_proposition, color:'#2d5a27' },
//     { label:'Key Differentiators', content: result.differentiators, color:'#1a4a7a' },
//     { label:'Call to Action', content: result.call_to_action, color:'#6b3a8a' },
//   ].filter(s => s.content) : []
...
//                   <button onClick={() => copy(content)} style={{ background:'transparent', border:'none', cursor:'pointer', color:'rgba(26,23,20,0.3)', display:'flex', alignItems:'center', gap:4, fontSize:11, transition:'color 0.15s' }}
//                     onMouseEnter={e => e.currentTarget.style.color='#1a1714'}
//                     onMouseLeave={e => e.currentTarget.style.color='rgba(26,23,20,0.3)'}
//                   >
//                     {copied ? <Check size={12} color="#2d5a27" /> : <Copy size={12} />}
//                   </button>
//                 </div>
...
//                   <button onClick={() => copy(result.full_pitch)} style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(245,240,232,0.1)', border:'1px solid rgba(245,240,232,0.15)', borderRadius:100, padding:'6px 14px', fontSize:11, color:'rgba(245,240,232,0.6)', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
//                     <Copy size={11} /> Copy full pitch
//                   </button>
//                 </div>
...
//       // ── Sections definition
//       const sectionDefs = [
//         { label: 'Elevator Pitch',      content: result.elevator_pitch,   color: [201, 79, 30],   lightColor: [255, 243, 237] },
//         { label: 'Value Proposition',   content: result.value_proposition, color: [45, 90, 39],    lightColor: [237, 247, 237] },
//         { label: 'Key Differentiators', content: result.differentiators,   color: [26, 74, 122],   lightColor: [235, 242, 252] },
//         { label: 'Call to Action',      content: result.call_to_action,    color: [107, 58, 138],  lightColor: [245, 238, 252] },
//         { label: 'Full Pitch',          content: result.full_pitch,        color: [26, 23, 20],    lightColor: [245, 240, 232] },
//       ].filter(s => s.content)
...
//         // Check if block fits, otherwise new page
//         if (y + blockH > pageH - 30) {
//           doc.addPage()
//           y = margin
//         }

//         doc.setFillColor(...section.lightColor)
...
//       const fileName = `${(result.product || 'pitch').replace(/\s+/g, '_')}_sales_pitch.pdf`
//       doc.save(fileName)
...
//                 {downloadingPDF ? <><span className="spin">⟳</span> Preparing PDF...</> : <><Download size={15} /> Download PDF</>}
//               </button>
//             )}
//           </div>
//         </div>
